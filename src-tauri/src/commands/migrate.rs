...
/// Zip the wiki/ directory into .llm-wiki/backups/migrate-schema-v1-<ts>.zip,
/// then enumerate every *.md under wiki/ (excluding .llm-wiki and backups).
///
...
pub fn migrate_wiki_backup(project_path: String) -> Result<BackupResult, String> {
    ...
    let backup_path = backups_dir.join(format!("migrate-schema-v1-{}.zip", ts));
        ...
        .filter(|e| e.file_type().is_file())
    {
        ...
        if rel_str.starts_with(".llm-wiki/") || rel_str.contains("/backups/") {
            continue;
        }
        files.push(rel_str);
    }
...
fn create_zip(source_dir: &Path, target_zip: &Path) -> Result<usize, String> {
    ...
    writer
        .finish()
        .map_err(|e| format!("WRITE_FAILED 关闭 zip 失败: {}", e))?;
    Ok(count)
}
