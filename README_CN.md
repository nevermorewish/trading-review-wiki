...
- **OpenClaw 协作** — 对接 OpenClaw 多 Agent 系统，每日 18:00 自动生成六部分完整复盘报告。详见 [collab/README.md](collab/README.md)。
...
**部署：** 将 `collab/` 目录复制到 Wiki 工作区 `raw/openclaw数据/`，修改 `_config.md` 中的路径，在 OpenClaw 中配置定时任务。完整说明见 [collab/README.md](collab/README.md)。
...
- **Wiki Doctor** —— 自动扫描并重组 wiki 目录结构：统一混合格式链接、将根目录松散文件移动到分类目录、检测拼音文件名建议重命名、冲突文件手动确认、操作前自动备份
